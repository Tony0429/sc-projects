...
def add_file(name_data, filename):
    ...
    with open(filename, "r") as f:
        for line in f:
            line_year = line.split()
            line_split = line.split(",")
            if line_year[0].isdigit() and int(line_year[0]) > 1000:
                year = line_year[0]
            elif len(line_split) >= 3:
                rank = line_split[0]            # 找出每個名字的rank
                name1 = line_split[1]           # 找出男女生的名稱
                name2 = line_split[2]
                # 確保空格被刪除
                rank = rank.strip()
                name1 = name1.strip()
...
def search_names(name_data, target):
    ...
    for name in name_data:
        ...
        for i in range(len(name)-len(target)+1):
            target_find = name[i: i+len(target)]
            if target_find == target:
                if name not in matching_names:
                    for j in range(len(name)):
                        selected_name = name[0].upper() + name[1:j+1]
                    matching_names.append(selected_name)
